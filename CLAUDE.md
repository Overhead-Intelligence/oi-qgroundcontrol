...
Current version: 1.0.0 (released 2026-09-17). Upstream base: the QGroundControl
**5.0 line**, upstream branch `Stable_V5.0` (v5.0.8 plus maintenance fixes, tip
`0e7d0e3b4`). v1.0.0 itself was built on v5.1.4 and is superseded: Roger
rejected the 5.1 interface ("tons of crazy changes"), so the fork moved back to
the line the May 2026 OI build used. Never move the base without asking him.
...
- `custom/src/OIPlugin.{h,cc}` — the `QGCCorePlugin` subclass. Hooks used (5.0
  signatures): `adjustSettingMetaData` (defaults read from `res/OI-defaults.ini`;
  returns the visibility bool), `factValueGridCreateDefaultSettings` (telemetry
  bar), `init` (deploys `res/OI-Actions.json`, creates the keyboard controller,
  registers the `OI.Controls` QML singleton), `createQmlApplicationEngine` /
  `cleanup` (URL interceptor), `firstRunPromptStdIds` (empty: no first-run
  Preferences prompt), `mavlinkMessage` (altitude error for the keyboard panel).
  The constructor runs `_importLegacySettings`: once per settings file, if the
  file is fresh, it copies every settings group and top-level key from
  `%APPDATA%\QGroundControl\QGroundControl OI Build.ini` (else `QGroundControl.ini`,
  both the same QGC generation) except `MavlinkActions` and `SettingsVersion`,
...
- `custom/src/qml/FlyViewCustomLayer.qml` — Fly view overlay (keyboard panel);
  the module is `QGroundControl.FlightDisplay` on the 5.0 line.
...
- `custom/build-config.json` — Qt version/modules and GStreamer version, read by
  the workflow and the local build scripts. Keep it in step with upstream
  `Stable_V5.0`'s `.github/workflows/windows.yml` when syncing.
- `.github/workflows/oi-windows.yml` — the only workflow; its steps mirror
  upstream `Stable_V5.0`'s `windows.yml`. `.github/actions/*` and
  `.github/scripts/*` are upstream's, reused unchanged.
...
- CI (about 30 min) is the compile check for every PR; download the artifact
  to test. For iteration, build locally: `custom/scripts/install-qt.cmd`
  (once), `build-local.cmd`, `run-local.cmd` (VS 2022 Build Tools with the C++
  workload, Python 3, Qt 6.8.3 from `custom/build-config.json` into `.qt/`).
  Local builds are video OFF unless the GStreamer 1.22.12 SDK is installed.
  Non-tag builds run as "QGroundControl-OI Daily" with separate settings; only
  tag builds set `QGC_STABLE_BUILD`.
...
`git fetch upstream Stable_V5.0`, branch `chore/upstream-stable-5.0` off
`development`, `git merge upstream/Stable_V5.0`, keep ours for `README.md`,
`CHANGELOG.md` and `.github/`, open a PR. Never merge `upstream/master` or a
newer `Stable_V5.x` branch: the QGC line is Roger's decision (see the top).
History note: `development` carries the v5.1.4-based v1.0.0 commits; the
switch back to 5.0 was a merge whose tree is the 5.0 tree, so no history was
rewritten and later `Stable_V5.0` merges apply cleanly.
...
- Local Qt installs use a pinned aqtinstall commit (in
  `custom/scripts/install-qt.cmd`); released aqtinstall versions lag behind new
  Qt repository layouts.
