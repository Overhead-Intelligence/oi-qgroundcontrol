...
// Both are the same QGC generation as this build, so every group is copied as is, except
// the actions file selection (the OI actions file bundled here supersedes it).
constexpr const char *kLegacyOrgName = "QGroundControl";
constexpr const char *kImportMarkerKey = "OI/importedSettingsFrom";
constexpr const char *kSettingsVersionKey = "SettingsVersion";
...
const QStringList kImportSkipGroups = {
    QStringLiteral("MavlinkActions"),
    QStringLiteral("OI"),
};
...
void OIPlugin::init()
{
    ...
    // Created here, after the settings system exists, because its settings group needs it.
    // Registered before the QML engine exists so FlyViewCustomLayer can "import OI.Controls"
    // and use the OIKeyboard singleton.
    _keyboard = new OIKeyboardController(this);
    (void) qmlRegisterSingletonInstance("OI.Controls", 1, 0, "OIKeyboard", _keyboard);
}

void OIPlugin::cleanup()
{
...
bool OIPlugin::adjustSettingMetaData(const QString &settingsGroup, FactMetaData &metaData)
{
    const bool visible = QGCCorePlugin::adjustSettingMetaData(settingsGroup, metaData);

    // Settings with an empty group name (the App group) sit in the [General] section of
...
void OIPlugin::_importLegacySettings()
{
    ...
    for (const QString &legacyApp : kLegacyAppNames) {
        ...
        for (const QString &group : groups) {
            if (kImportSkipGroups.contains(group)) {
                continue;
            }
            copied += copySettingsGroup(legacy, settings, group);
        }

        // Top-level keys are the [General] section: save path, GCS system ID, offline
        // editing defaults, first-run prompts already shown, and so on.
        const QStringList keys = legacy.childKeys();
        for (const QString &key : keys) {
            if (key == QLatin1String(kSettingsVersionKey)) {
                continue;
            }
